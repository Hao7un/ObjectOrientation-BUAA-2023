...
public class Parser {
    ...
    public Factor parseFactor() {
        if (lexer.peek().equals("(")) {      //expr
            lexer.next();
            return parseExpr();
        } else if (lexer.peek().equals("x") ||
                lexer.peek().equals("y") ||
                lexer.peek().equals("z")) {  //pow
            return parsePow();
        } else if (lexer.peek().equals("sin") ||  //trigono
                lexer.peek().equals("cos")) {
            return parseTri();
        } else if ("fgh".indexOf(lexer.peek().charAt(0)) != -1) {
            return parseFunction();
        } else if (lexer.peek().equals("d")) { //derivation
    ...
    public Factor parseDerivation() {
        lexer.next();
        String var = lexer.peek(); //get x/y/z
        lexer.next();
        lexer.next();//pass (
        Expr temp = parseExpr();
        temp = temp.diff(var);
        Derivation derivation = new Derivation(var,temp);
        return derivation;
    }
