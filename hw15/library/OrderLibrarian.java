...
public class OrderLibrarian {  //student-->data-->order
    ...
    public RequestTag orderNewBook(Request request,ArrayList<String> orderInformation) {
        Student student = request.getStudent();
        String type = request.getBook().getType();
        String bid = request.getBook().getId();
        String sid = student.getId();
        Book book = new Book(type,bid,libraryId,-1);
        ...
        orderInformation.add(String.format("(Sequence) [%4d-%02d-%02d] OrderLibrarian " +
                "sends a message to Library",request.getYear(),request.getMonth(),
                request.getDay()));
    ...
    public void getOrderedBook(ArrayList<Integer> date,
        ...
        for (Iterator<Order> iterator = orderList.iterator(); iterator.hasNext();) {
            ...
            if (orderedBooks.containsKey(order.getBook())) {
                ...
                book.setBorrowedDays(currentDays); //记录下来借的日期！！
                ...
                System.out.printf("(State) [%04d-%02d-%02d] %s-%s transfers from " +
                        ...
                        book.getType(),book.getId());
                System.out.printf("(Sequence) [%04d-%02d-%02d] Machine sends " +
